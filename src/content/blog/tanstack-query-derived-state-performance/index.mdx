...
- **Cross-source composition** (combine multiple queries) uses `fetchQuery`-in-`queryFn`, because `select` only ever sees one query's data. The price is that the derived entry caches a snapshot and does not auto-recompute when its sources change, so I own its invalidation.
...
- **I predicted invalidation matching would be the next hotspot, and measurement proved me wrong.** For a while, the largest remaining TanStack cost in my profiles was `partialMatchKey`/`matchQuery` walking the query cache, and I assumed the `level:N` cascade was the culprit, since `invalidateQueries` is O(cache size) per call and the cascade calls it once per level, per address. When I finally attributed the cost properly (caller chains, not just self time), the cascade measured ~20ms. The seconds (3.2 of them on a cold start) came from a single `useIsFetching({ predicate })` in the header's refresh button: TanStack re-evaluates that predicate against **every cache entry on every cache event**, for as long as the component is mounted, and my predicate did an O(addresses) `includes` per entry on top. Replacing it with a small incremental cache subscription (O(1) per event) took the scans to zero samples, and, as a bonus, collapsed the `notifyManager` timer churn, since each of those events had also been scheduling a `setTimeout(0)` notification flush. The general lesson: 
