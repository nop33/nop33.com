...
import imgDesktopWallet2 from './desktop-wallet-2.png'
import imgArchitectureDiagram from './architecture-diagram.svg'
import imgDesktopWallet1 from './desktop-wallet-1.png'
import imgDesktopWallet4 from './desktop-wallet-4.png'
...
<BreakoutSection>
  <div class="my-12 md:my-20">
    <Media src={imgDesktopWallet2.src} />
  </div>
</BreakoutSection>
...
<div>
  <div class="my-20 md:my-32 text-center">
    ...
    <BreakoutSection>
      <div class="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-3 my-12 md:my-20 text-left">
...
<BreakoutSection>
  <div class="my-12 md:my-20">
    <Media src={imgArchitectureDiagram.src} />
  </div>
</BreakoutSection>

<BreakoutSection>
  <div class="my-12 md:my-20 space-content">
    ## Architecture
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
      ...
      <div class="space-content">
        ...
        - **Signing**: signing with stored mnemonic or with hardware wallets.
        - **Address generation**: discovering active addresses and deterministically regenerating addresses from stored metadata.
      </div>
...
<BreakoutSection>
  <div class="my-20 md:my-32">
    <Media src={imgDesktopWallet1.src} />
  </div>
</BreakoutSection>
...
The current architecture includes Redux for UI state and Tanstack Query for server state. We migrated the server state from Redux to Tanstack Query to benefit from several features such an async library provides, such as a request retry mechanism and caching. This has improved the performance of the app even further. It also disentangled the logic of how and when API requests are fired, something that it was quite challenging to achieve with Redux thunks. On the downside, the developer now has to maintain two mental models and two debugging interfaces. All read-flows are now declarative through the Tanstack Query hooks, but the write flows remain imperative. I documented the whole migration, with before-and-after profiling data, in my [five-part TanStack Query case study](/blog/tanstack-query-wallet-async-state).
...
<BreakoutSection>
  <div class="my-20 md:my-32">
    <Media src={imgDesktopWallet4.src} />
  </div>
</BreakoutSection>

## Challenges & lessons learned

This project has its fair share of challenges and lessons learned. From [secrets management in process memory](/blog/clearing-secrets-from-memory) to API rate limiting, throttling and caching, there was and still is a lot to learn from this project. I cover the async-state journey in depth in my [five-part TanStack Query case study](/blog/tanstack-query-wallet-async-state).

### Early misuse of React Context

Storing API data in React Context led to excessive re-renders. The defined `Map` object turned out not to be the appropriate data structure to store the remote addresses data, since a new object would be defined for every data change of any address of the wallet, causing components to re-render even though they didn't have to. The solution was to migrate the data layer to Redux.
...
The solution came in many stages, each covered in detail in my [TanStack Query case study](/blog/tanstack-query-wallet-async-state):

- Backend and frontend [rate limiting](/blog/tanstack-query-wallet-async-state).
- Smarter retry handling.
- [Proper caching with TanStack Query](/blog/redux-to-tanstack-query-migration).
- A ["latest transaction" sentinel query](/blog/tanstack-query-change-detection-polling) to control when other queries should refresh.
- [Limiting background refreshes](/blog/tanstack-query-change-detection-polling) to the most active addresses.
- [Persisting query caches across sessions](/blog/tanstack-query-persist-cache-cold-start).

Each step reduced load and improved UX, especially for power users with large wallets.

### Startup performance

Persisted caches solved cold-start slowness, but continuous throttled persistence had problems of its own: it wastefully re-serialized the entire cache on every poll tick, and, more importantly, its deferred writes could lose the race against process suspension and drop data. The final solution was to persist the cache synchronously at well-defined lifecycle points (before quit), rather than continuously. I cover the reasoning in [Part 5 of my TanStack Query series](/blog/tanstack-query-persist-cache-cold-start).

<div>
  <div class="my-20 md:my-32 space-content">
