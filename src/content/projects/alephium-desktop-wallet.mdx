...
I worked on the wallet as a core frontend engineer, contributing both to product features and to foundational architectural decisions together with a colleague. Over time, the project evolved from a relatively simple Electron app into a highly modular, performance-sensitive system that balances UX, security, and scalability.
...
<BreakoutSection>
  <div class="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-3">
    <Feature title="Create and manage multiple Alephium wallets">Encrypted and stored locally.</Feature>
    <Feature title="Generate multiple addresses per wallet">With deterministic restoration.</Feature>
    <Feature title="Send transactions">
      multiple assets and NFTs per transaction, optional unlock dates, configurable gas settings
    </Feature>
    <Feature title="View, filter, and export transaction history" />
    <Feature title="Create passphrase-wallets">For advanced security and plausible deniability.</Feature>
    <Feature title="Connect Ledger hardware wallets for cold signing" />
    <Feature title="Connect to Alephium dApps via WalletConnect">
      For seamless integration with the Alephium ecosystem.
    </Feature>
    <Feature title="Offline / air-gapped wallet generation" />
    <Feature title="Import wallets created with other Alephium software" />
    <Feature title="Export wallets via animated QR codes">Compatible with the Alephium Mobile Wallet.</Feature>
    <Feature title="Extensive personalization">
      localization (language, currency, number formats), theming (light / dark / system), configurable node endpoints
      and proxy settings
    </Feature>
  </div>
</BreakoutSection>
...
import type BreakoutSection from "@components/BreakoutSection.astro"
